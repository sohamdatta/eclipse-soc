...
public class PluginVisualizationView extends ViewPart implements IZoomableWorkbenchPart {
	...
	private void makeActions() {
		...
		expandNodeAction = new Action() {
			public void run() {
				Object element = ((IStructuredSelection) viewer.getSelection()).getFirstElement();
				GraphItem graphItem = viewer.findGraphItem(element);
				Graph g = (Graph) viewer.getControl();
				g.setExpanded((GraphNode) graphItem, true);
			}
		};
		expandNodeAction.setText("expand node");
		expandNodeAction.setToolTipText("Reveal children of this plugin");
		expandNodeAction.setEnabled(true);
		
		collapseNodeAction = new Action() {
			public void run() {
				Object element = ((IStructuredSelection) viewer.getSelection()).getFirstElement();
				GraphItem graphItem = viewer.findGraphItem(element);
				Graph g = (Graph) viewer.getControl();
				g.setExpanded((GraphNode) graphItem, false);
			}
		};
		collapseNodeAction.setText("collapse node");
		collapseNodeAction.setToolTipText("Hide children of this plugin");
		collapseNodeAction.setEnabled(true);
	}
		...
		if (((IStructuredSelection) viewer.getSelection()).size() == 1) {
			...
			if (graphItem instanceof GraphNode) {
				...
				if (g.canCollapse(node))
					manager.add(collapseNodeAction);
			}
		}
