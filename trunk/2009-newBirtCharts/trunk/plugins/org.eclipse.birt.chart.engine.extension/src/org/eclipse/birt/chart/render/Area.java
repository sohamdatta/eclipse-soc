...
public class Area extends Line
{
			...
			throws ChartException
	{
		...
		else
		{
			...
			if ( ChartUtil.isStudyLayout( getModel( ) ) )
			{
				...
				zeroLocation = Math.floor( srh.getLocationOnOrthogonal( new Double( dStart ) ) );
			}
		...
		List fixedPoints = new ArrayList( );
		...
		else
		{
			...
			{
				tapeColor = ( (Gradient) fillColor ).getStartColor( )
						.brighter( );
			...
			if ( findex != -1 )
			{
				...
				if ( lia.isVisible( ) )
				{
					...
					for ( int i = 0; i < flst.size( ) - 1; i++ )
					{
						...
						double[] pt = (double[]) flst.get( i );
						start.set( pt[0], pt[1] );
						pt = (double[]) flst.get( i + 1 );
						end.set( pt[0], pt[1] );
				...
				if ( fixedPoints.size( ) > 0 )
				{
					if ( isTransposed )
					{
						lst.add( goFactory.createLocation( zeroLocation,
								( (double[]) fixedPoints.get( fixedPoints.size( ) - 1 ) )[1] ) );
						lst.add( goFactory.createLocation( zeroLocation,
								( (double[]) fixedPoints.get( 0 ) )[1] ) );
					}
					else
					{
						lst.add( goFactory.createLocation( ( (double[]) fixedPoints.get( fixedPoints.size( ) - 1 ) )[0],
								zeroLocation ) );
						lst.add( goFactory.createLocation( ( (double[]) fixedPoints.get( 0 ) )[0],
								zeroLocation ) );
					}
				}
	...
	private int getFirstDupIndex( List fixedPoints, boolean checkX )
	{
	...
	private int[] getRealIndexList( List fixedPoints, boolean checkX )
	{
			...
			Fill fPaletteEntry, Bounds bo ) throws ChartException
	{
		...
		if ( liaMarker.isVisible( ) )
		{
					...
					&& ls.isPaletteLineColor( ) )
			{
				...
				liaMarker.setColor( goFactory.copyOf( (ColorDefinition) fPaletteEntry ) );
			}
	...
	private static class Envelop
	{
		...
		static class ChangeItem
		{
			...
			public ChangeItem( Point pt0, Point pt1 )
			{
				if(pt0==null)
				{
					pt0 = null;
				...
				double x1, double y1, double dTapeWidth )
		{
			...
			loa[0] = goFactory.createLocation( x0, y0 );
			loa[1] = goFactory.createLocation( x1, y1 );
			loa[2] = goFactory.createLocation( x1 + dTapeWidth, y1 - dTapeWidth );
			loa[3] = goFactory.createLocation( x0 + dTapeWidth, y0 - dTapeWidth );
			return loa;
		}
